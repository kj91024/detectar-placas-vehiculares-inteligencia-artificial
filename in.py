...
from util import (obtenerTextoDesdeImagen, pantallaDeNoDeteccion, obtenerCarro,
                  detectarVehiculos, trackearVehiculos, procesarPlacaVehicular,
                  dibujarPlaca, totalVideos)
from tkinter import *
...
total_videos = totalVideos('datasets/for_detections/videos/in')
is_video = False
index_video = 1
detalle_placas = {}
...
def video_stream():
    ...
    if frame_nmr == ((fps * index_video) + (fps * (index_video-1))) and index_video <= total_videos:
        ...
        video_path = 'datasets/for_detections/videos/in/' + str(index_video) + '.mp4'
            ...
            for license_plate in license_plates.boxes.data.tolist():
                ...
                if ((car_id in detalle_placas) and (frame_nmr == _frame_nmr + analisis_frame or frame_nmr == _frame_nmr + total_frames - 1)):
                    ...
                    for item in plaquitas.keys():
                        ...
                        _placa = item
                        break
                    response = requests.get('http://localhost:8080/ingreso/' + _placa)
                    print(response)
        else:
